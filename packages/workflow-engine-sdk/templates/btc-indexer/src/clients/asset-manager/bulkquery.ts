...
export type DataModelQuery = {
  skip?: number;
  limit?: number;
  /** Request the total count of matching items (adds a COUNT query). */
  count?: boolean;
  sort?: string[];
  ascending?: boolean;
  descending?: boolean;
...
export type FilterResult<T> = {
  count: number;
  /** Total number of matching items; only present when count=true was requested. */
  total?: number;
  /** True when the full result set was returned with no further pages. */
  allItems: boolean;
