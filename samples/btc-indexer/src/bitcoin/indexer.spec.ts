...
describe('BTCIndexer.setup()', () => {
  it('upserts asset, address, and pool with create_or_ignore', async () => {
    ...
    await indexer.setup(client as any, BTC_CONFIG);
...
describe('BTCIndexer.process()', () => {
  ...
  beforeEach(async () => {
    ...
    await indexer.setup(mockClient as any, BTC_CONFIG);
    vi.clearAllMocks();
  });

  it('makes no calls for an empty batch', async () => {
    const result = await (indexer as any).process([]);
  ...
  it('throws on network name mismatch', async () => {
    ...
    await expect((indexer as any).process([event])).rejects.toThrow(/Network mismatch/);
  });
  ...
  it('upserts input and output fragments when no wallets are involved', async () => {
    ...
    await (indexer as any).process([event]);
  ...
  it('returns a checkpoint with lastPollTime after processing events', async () => {
    ...
    const result = await (indexer as any).process([event]);
