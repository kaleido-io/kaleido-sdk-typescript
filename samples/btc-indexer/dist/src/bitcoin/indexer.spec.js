...
function makeVIn(txid, vout) {
    return { txid, vout, scriptSig: { hex: '' }, sequence: 0xffffffff };
}
function makeVOut(n, valueSat, address) {
    return {
        n,
        valueSat,
...
describe('BTCIndexer.setup()', () => {
    it('upserts asset, address, and pool with create_or_ignore', async () => {
        const client = { bulkUpsert: vi.fn().mockResolvedValue({}), bulkQuery: vi.fn() };
        const indexer = new BTCIndexer();
        await indexer.setup(client, BTC_CONFIG);
...
describe('BTCIndexer.process()', () => {
    ...
    beforeEach(async () => {
        ...
        await indexer.setup(mockClient, BTC_CONFIG);
        vi.clearAllMocks();
    });
    ...
    it('throws on network name mismatch', async () => {
        ...
        await expect(indexer.process([event])).rejects.toThrow(/Network mismatch/);
    });
    ...
    it('creates a transfer with a subtract balance change for a known input wallet', async () => {
        const prevUtxoInfo = { n: 0, valueSat: 50000, scriptPubKey: { hex: '', address: 'addr2', type: 'p2pkh' } };
        const event = makeEvent(makeTxEvent({
            txid: 'tx003',
            vin: [makeVIn('prevtx', 0)],
    ...
    it('creates separate transfers for sender and receiver wallets', async () => {
        ...
        const senderXfer = transfers.find((t) => t.protocolId === 'tx004.wallet2');
