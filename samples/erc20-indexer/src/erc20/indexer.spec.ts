...
describe('ERC20Indexer.indexBatch()', () => {
  ...
  beforeEach(async () => {
    ...
    indexer = new ERC20Indexer(MOCK_INDEXER_CONFIG);
    await indexer.setup(ERC20_CONFIG, mockClient);
    vi.clearAllMocks(); // clear the setup() bulkUpsert call
  });

  it('does not call bulkUpsert for an empty batch', async () => {
    await indexer.indexBatch(mockReqContext, [], mockClient);
  ...
  it('does not call bulkUpsert when decodedEvents is absent', async () => {
    ...
    await indexer.indexBatch(mockReqContext, [makeEvent(tx)], mockClient);
    expect(mockClient.bulkUpsert).not.toHaveBeenCalled();
  });
  ...
  it('processes a regular transfer correctly', async () => {
    ...
    const result = await indexer.indexBatch(mockReqContext, [makeEvent(tx)], mockClient);
  ...
  it('deduplicates addresses across multiple transfers in one batch', async () => {
    ...
    await indexer.indexBatch(mockReqContext, events, mockClient);
