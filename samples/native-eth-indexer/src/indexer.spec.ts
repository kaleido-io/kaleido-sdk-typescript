...
// import type { ETHTransactionEvent, TxSummaryVIn, TxSummaryVOut } from '@kaleido-io/workflow-engine-sdk/types/eth';
...
// function makeEvent(ethTx: ETHTransactionEvent): EventProcessorEvent<ETHTransactionEvent> {
//   return { idempotencyKey: ethTx.tx.txid, topic: 'ethTransactions', data: ethTx };
// }
...
//     await indexer.setup(ETH_CONFIG, client as any);

//     expect(client.bulkUpsert).toHaveBeenCalledTimes(1);
//     const payload = client.bulkUpsert.mock.calls[0][0];

//     expect(payload.assets).toHaveLength(1);
//     expect(payload.assets[0]).toMatchObject({ name: 'eth', updateType: 'create_or_ignore' });

//     expect(payload.addresses).toHaveLength(1);
//     expect(payload.addresses[0]).toMatchObject({ address: 'eth', contract: true, updateType: 'create_or_ignore' });

//     expect(payload.pools).toHaveLength(1);
//     expect(payload.pools[0]).toMatchObject({ name: 'eth', asset: 'eth', standard: 'eth', updateType: 'create_or_ignore' });
//   });
// });
...
//     await indexer.setup(ETH_CONFIG, mockClient as any);
//     vi.clearAllMocks();
//   });
...
//       operation: 'add',
//     });
//     expect(transfers[0].to).toBe('eth_wallet1');
//   });
...
//       operation: 'subtract',
//     });
//     expect(transfers[0].from).toBe('eth_wallet2');
//   });
